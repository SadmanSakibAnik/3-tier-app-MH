...
import { ShieldCheck, LayoutDashboard, PlusCircle, User, LogOut, Menu, X } from "lucide-react"
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    ...
    { href: "/profile", label: "Profile & Addresses", icon: User },
  ]
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="hidden w-64 border-r border-slate-200 bg-white md:block">
        <div className="flex h-16 items-center gap-2 border-b border-slate-100 px-6">
          <div className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 text-white">
            <ShieldCheck className="h-5 w-5" />
          </div>
          <span className="text-lg font-bold text-slate-800">Mehedi Portal</span>
        </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-slate-600 transition-all hover:bg-violet-50 hover:text-violet-600"
                >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-6 md:justify-end">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 w-8 rounded-full bg-gradient-to-tr from-violet-600 to-indigo-500 text-white flex items-center justify-center font-bold text-sm">
              {user.name.charAt(0)}
            </div>
          </div>
        </header>
